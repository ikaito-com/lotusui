...
- CORNER RADIUS never exceeds HALF the shorter side. Past that a
  rounded rect is degenerate, and widget.Border strokes the broken
  path ACROSS THE WHOLE WINDOW, not just the widget (this shipped
  once, as a 999dp "make it a pill" shortcut on the docs platform
  chips). A pill is radius = height/2, not a big constant; run any
  size-varying radius through ClampCorner(r, size); circles paint two
  discs rather than a border. Shadow helpers grow rect and radius
  together, so they are safe only if the caller's radius already is.
