// Command gallery is the ONE lotusui demo app: every component the docs
// site shows live is a state in this binary, addressed by URL hash
// (#button, #modal/open, …). It compiles once to WebAssembly and is
// embedded by the docs as one iframe per Preview (or opened standalone
// at /gallery/); natively it doubles as the component test bed
// (LOTUSUI_GALLERY=modal/open go run ./gallery), a state harness for
// screenshots — every state reachable without click choreography.
package main
...
func main() {
	go func() {
		w := new(app.Window)
		w.Option(app.Title("lotusui gallery"), app.Size(900, 680))
...
// th is swappable along TWO orthogonal axes: the palette picker sends
// a color preset, the look picker a look-and-feel preset (font,
// spacing, radii, text size). Selection composes them into one
// ordinary NewTheme call — the same multi-theme pattern an app
// offers its users, demonstrated live.
var (
	th         = lotusui.NewTheme()
	curPalette = "lavender"
	curLook    = "lotus"
...
func rebuildTheme() {
	nt := looks.ByName(curLook).Theme(palettes.ByName(curPalette).Palette)
	nt.UpgradeShaperAsync(func() {})
	th = nt
}
...
func loop(w *app.Window) error {
	...
	// Wheel router must define lotusuiSetRegions before the docs page
	// answers lotusuiReady with a regions payload.
	initWheelRouter()
	initOverlay(w.Invalidate)
	...
	for {
		...
		case app.FrameEvent:
			...
			if len(pendingMeasureSpecs) > 0 {
				sendMeasured(measureStates(th, gtx, pendingMeasureSpecs, pendingMeasureWidths))
				pendingMeasureSpecs, pendingMeasureWidths = nil, nil
			}
			dims := render(th, gtx)
			reportRegionEdges()
			...
			// Tell the embedding docs page our real content height so
			// the iframe hugs the demo and the PAGE keeps scrolling —
			// no scrollbars-within-scrollbars.
			reportHeight(dims.Size.Y)
			e.Frame(gtx.Ops)
		}
	}
}

// A demo is one addressable gallery entry. render lays out the demo's
// states inside the page column; overlay (optional) runs at WINDOW
// constraints after the page — the modal's scrim needs the whole
// window, exactly the constraint trap Modal's doc warns about.
type demo struct {
	slug       string
...
func render(th *lotusui.Theme, gtx C) D {
	...
	// is one box and one scrollbar — the page's.
	if d.slug != "blank" {
	...
	dims := layout.UniformInset(lotusui.Space.LG).Layout(cgtx, func(gtx C) D {
		return d.render(th, gtx)
	})
	if d.overlay != nil {
		d.overlay(th, gtx) // overlays (the modal scrim) stay at WINDOW constraints
	}
	return dims
}
...
func card(th *lotusui.Theme, gtx C, sections ...layout.Widget) D {
	...
	return lotusui.VStack(lotusui.Space.LG, sections...)(gtx)
}
...
func tileGrid(gtx C, items []tileItem) D {
	return lotusui.SimpleGrid(th, gtx, items, lotusui.SimpleGridProps{
